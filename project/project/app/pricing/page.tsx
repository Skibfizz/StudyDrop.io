...
export default function PricingPage() {
  ...
  const handleSubscribe = async (planType: 'free' | 'basic' | 'pro') => {
    try {
      ...
      if (!user) {
        // Save the selected plan and redirect to sign in
        localStorage.setItem('selectedPlan', planType);
        router.push(`/auth/signin?redirect=/pricing&plan=${planType}`);
        return;
      }
      ...
      if (planType === 'free') {
        ...
        toast({
          ...
          description: "You're now on the free plan. Enjoy our basic features!",
          variant: "success"
        });
      ...
      const response = await fetch('/api/stripe/create-checkout', {
        ...
        body: JSON.stringify({
          priceId: planType === 'pro' ? process.env.NEXT_PUBLIC_STRIPE_PRO_PRICE_ID : process.env.NEXT_PUBLIC_STRIPE_BASIC_PRICE_ID,
        }),
      });
                ...
                >
                  ...
                  <CardContent className="relative space-y-8">
                    <Button 
                      ...
                      onClick={() => handleSubscribe(plan.name.toLowerCase() as 'free' | 'basic' | 'pro')}
